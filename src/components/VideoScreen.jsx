  ...
  background: rgba(148,163,184,0.3);
      ...
      `}</style>
            ...
            <div className="vscreen-root">
                <div className="max-w-2xl mx-auto">
                    ...
                    >
                        ...
                        {/* Materials Button */}
                        {(currentVideo?.pdfUrl || currentVideo?.pptxUrl || currentVideo?.notes) && (
                            <div className="px-4 pb-2">
                                <div className="vs-desc flex flex-wrap gap-2">
                                    <p className="w-full text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">
                                        {isAr ? 'المواد التعليمية' : 'Study Materials'}
                                    </p>
                                    {currentVideo.pdfUrl && (
                                        <button
                                            onClick={() => setViewerUrl({ url: currentVideo.pdfUrl, type: 'pdf' })}
                                            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-500/10 border border-red-500/25 text-red-400 font-semibold text-sm hover:bg-red-500/20 hover:scale-105 transition-all">
                                    ...
                                    {currentVideo.pptxUrl && (
                                        <button
                                            onClick={() => setViewerUrl({ url: currentVideo.pptxUrl, type: 'pptx' })}
                                            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-orange-500/10 border border-orange-500/25 text-orange-400 font-semibold text-sm hover:bg-orange-500/20 hover:scale-105 transition-all">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-4 h-4">
                                                <rect x="2" y="3" width="20" height="14" rx="2" /><line x1="8" y1="21" x2="16" y2="21" /><line x1="12" y1="17" x2="12" y2="21" />
                                            </svg>
                                            {isAr ? 'عرض البوربوينت' : 'View PPTX'}
                                        </button>
                                    )}
                                    {currentVideo.notes && (
                                        <button
                                            onClick={() => setShowNotes(prev => !prev)}
                                            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-cyan-500/10 border border-cyan-500/25 text-cyan-400 font-semibold text-sm hover:bg-cyan-500/20 hover:scale-105 transition-all">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-4 h-4">
                                                <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" />
                                                <line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" />
                                            </svg>
                                            {isAr ? 'عرض الشرح النصي' : 'Show Notes'}
                                        </button>
                                    )}
                                    {currentVideo.notes && showNotes && (
                                        <div className="w-full mt-2 p-3 rounded-xl bg-slate-900/60 border border-slate-700/50 text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">
                                            {currentVideo.notes}
                                        </div>
                                    )}
                                </div>
            ...
            {viewerUrl && (
                ...
                >
                    <div
                        className="vs-card rounded-2xl overflow-hidden w-full max-w-4xl flex flex-col"
                        style={{ height: '85vh' }}
                        onClick={e => e.stopPropagation()}
                    >
                        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-800/80">
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full ${viewerUrl.type === 'pdf' ? 'bg-red-400' : 'bg-orange-400'}`} />
                                <span className="text-white font-semibold text-sm">
                                    {viewerUrl.type === 'pdf' ? (isAr ? 'عرض PDF' : 'PDF Viewer') : (isAr ? 'عرض البوربوينت' : 'PPTX Viewer')}
                                </span>
                            </div>
                            <div className="flex items-center gap-2">
                                <a
                                    ...
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-700/50 border border-slate-600/50 text-slate-300 text-xs font-semibold hover:bg-slate-700 transition-all"
                                >
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5">
                                        <path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6" />
                                        <polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" />
                                    </svg>
                                    {isAr ? 'فتح خارجي' : 'Open externally'}
                                </a>
                                <button
                                    onClick={() => setViewerUrl(null)}
                                    className="w-8 h-8 rounded-lg bg-slate-700/50 border border-slate-600/50 text-slate-300 flex items-center justify-center hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/30 transition-all"
                                >
